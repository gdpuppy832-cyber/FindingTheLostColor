...
public class BossAttack : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        phase1Attacks = new List<AttackEntry>
        {
            ...
            new AttackEntry("PortalSpawn", PortalSpawnAttackRoutine),
        };
    ...
    void Start()
    {
        ...
        // 시작 시 포탈 오브젝트가 지정되어 있다면 투명화 및 비활성화 초기 설정
        if (portalObjects != null)
        {
            for (int i = 0; i < portalObjects.Length; i++)
            {
                ...
                {
                    SetPortalAlpha(portalObjects[i], 0f);
                    portalObjects[i].SetActive(false);
                }
            }
        }
        ...
        // 게임 시작하고 1분 뒤 최초 소환이 시작되도록 마지막 소환 시간을 시작 시점으로 초기화 설정
        lastPortalSpawnTime = Time.time;
    }
    ...
    void Update()
    {
        ...
        AttackEntry chosen = null;

        // [신규] 1분(60초) 고정 주기 확정 소환 패턴 스케줄러 작동
        if (Time.time - lastPortalSpawnTime >= 60f)
        {
            chosen = FindAttackByName("PortalSpawn");
            if (chosen != null)
            {
                // 소환 타이머 초기화
                lastPortalSpawnTime = Time.time;
        ...
        // 1분 주기가 아직 도달하지 않았거나, 씬에 포탈 지정이 누락된 경우 다른 패턴 실행
        if (chosen == null)
        {
            List<AttackEntry> pool = GetCurrentPhasePool();
            if (pool == null || pool.Count == 0) return;

            chosen = PickRandomAttack(pool);
        }
    ...
    List<AttackEntry> GetCurrentPhasePool()
    {
        ...
        List<AttackEntry> rawPool = phase2Unlocked ? phase2Attacks : phase1Attacks;
        List<AttackEntry> filteredPool = new List<AttackEntry>(rawPool);

        // 무작위 공격 풀에서는 포탈 소환 패턴을 완전히 제외 (오직 1분 주기 시점에만 강제로 발동하게 통제)
        filteredPool.RemoveAll(attack => attack.name == "PortalSpawn");
    ...
    [Header("Portal Pattern Settings")]
    [Tooltip("씬에 포탈 오브젝트 2개")]
    public GameObject[] portalObjects;
    [Tooltip("1번째 소환 때 나올 몬스터 프리팹 2개 (2초 텀)")]
    public GameObject[] spawnMonsterPrefabs1;
    [Tooltip("2번째 소환 때 나올 몬스터 프리팹 2개 (2초 텀)")]
    public GameObject[] spawnMonsterPrefabs2;
    [Tooltip("포탈 소환 패턴의 락 아웃 시간 (다른 패턴 작동 차단 시간, 기본 4초)")]
    public float portalPatternCooldown = 4.0f;

    private float bossBattleStartTime;        // 보스전 시작 시간 기록용
    private int portalSpawnSequenceCount = 0; // 몇 번째 포탈 소환 공격인지 카운트 (1, 2, 1, 2 반복용)
    private float lastPortalSpawnTime;        // 마지막 포탈 소환 공격 시간 (1분 주기 연사용)
    ...
    // ================= 포탈 소환 패턴 (신규 패턴) =================
    IEnumerator PortalSpawnAttackRoutine()
    {
        // 1. 포탈 오브젝트 지정 여부 검사
        if (portalObjects == null || portalObjects.Length == 0)
        {
            Debug.LogError($"[PortalSpawn] portalObjects가 비어있거나 지정되지 않았습니다! (현재 크기: {(portalObjects != null ? portalObjects.Length.ToString() : "Null")})");
            yield break;
        }

        // 현재 몇 번째 소환인가에 따라 1차/2차 몬스터 리스트 스왑 (1->2->1->2 순환)
        GameObject[] currentSpawnPrefabs = (portalSpawnSequenceCount % 2 == 0) ? spawnMonsterPrefabs1 : spawnMonsterPrefabs2;
        string sequenceName = (portalSpawnSequenceCount % 2 == 0) ? "첫 번째 소환 패턴(1)" : "두 번째 소환 패턴(2)";

        // 2. 몬스터 프리팹 지정 여부 검사
        if (currentSpawnPrefabs == null || currentSpawnPrefabs.Length == 0)
        {
            Debug.LogError($"[PortalSpawn] {sequenceName}의 spawnMonsterPrefabs가 비어있거나 지정되지 않았습니다!");
            yield break;
        }

        int activePortalCount = portalObjects.Length;
        int monsterCount = currentSpawnPrefabs.Length;

        Debug.Log($"[PortalSpawn] {sequenceName} 포탈 {activePortalCount}개 활성화 및 페이드인 시작 (몬스터 {monsterCount}마리 순차 소환 대기)");

        // 1. 포탈 활성화 및 1.5초간 투명 -> 불투명 페이드인
        for (int i = 0; i < activePortalCount; i++)
        {
            if (portalObjects[i] != null)
            {
                SetPortalAlpha(portalObjects[i], 0f);
                portalObjects[i].SetActive(true);
            }
        }
        ...
        while (elapsed < fadeTime)
        {
            ...
            for (int i = 0; i < activePortalCount; i++)
            {
                if (portalObjects[i] != null)
                {
                    SetPortalAlpha(portalObjects[i], progress);
                }
            }
            yield return null;
        }
        ...
        for (int i = 0; i < activePortalCount; i++)
        {
            if (portalObjects[i] != null)
            {
                SetPortalAlpha(portalObjects[i], 1f);
            }
        }

        // 2. 지정한 몬스터 종류별로 양쪽 포탈에서 각각 1마리씩 총 2마리를 동시에 스폰 (2초 텀, 최종 총 4마리 소환)
        for (int i = 0; i < monsterCount; i++)
        {
            if (currentSpawnPrefabs[i] == null) continue;

            // 모든 활성화된 포탈(왼쪽, 오른쪽)에서 해당 몬스터 종류를 1마리씩 각각 소환
            for (int p = 0; p < activePortalCount; p++)
            {
                if (portalObjects[p] != null)
                {
                    GameObject spawnedMonster = Instantiate(currentSpawnPrefabs[i], portalObjects[p].transform.position, Quaternion.identity);
                    
                    // [신규 연출 연동] 정화 시 포탈 환원 연출 컴포넌트 주입 및 포탈 위치 정보 등록
                    PortalMonsterLinger linger = spawnedMonster.AddComponent<PortalMonsterLinger>();
                    linger.Setup(portalObjects[p].transform.position);
                }
            }

            Debug.Log($"[PortalSpawn] 양쪽 포탈에서 몬스터 '{currentSpawnPrefabs[i].name}'를 각각 1마리씩 총 {activePortalCount}마리 스폰 완료 ({(i + 1)}/{monsterCount})");

            // 마지막 소환 종류인 경우 2초 텀 대기를 건너뜀
            if (i < monsterCount - 1)
            {
        ...
        // 3. 몬스터 소환 완료 후 포탈이 1초간 다시 투명해지고 비활성화
        float fadeOutTime = 1.0f;
        float elapsedOut = 0f;
        while (elapsedOut < fadeOutTime)
        {
            elapsedOut += Time.deltaTime;
            float progress = Mathf.Clamp01(1f - (elapsedOut / fadeOutTime));
            for (int i = 0; i < activePortalCount; i++)
            {
                if (portalObjects[i] != null)
                {
        ...
        // 4. 소환 회수 시퀀스 카운트 증가 (1 -> 2 -> 1 -> 2 반복 스왑용)
        portalSpawnSequenceCount++;

        // 4. 패턴 종료 후 다른 패턴이 즉시 나가지 않도록 4초간 공격 대기 잠금 (쿨타임)
        nextAttackAllowedTime = Time.time + portalPatternCooldown;
    }

    private void SetPortalAlpha(GameObject portal, float alpha)
    {
        if (portal == null) return;
        SpriteRenderer[] renderers = portal.GetComponentsInChildren<SpriteRenderer>(true);
    ...
    private AttackEntry FindAttackByName(string name)
    {
        List<AttackEntry> rawPool = phase2Unlocked ? phase2Attacks : phase1Attacks;
        return rawPool.Find(attack => attack.name == name);
    }
}