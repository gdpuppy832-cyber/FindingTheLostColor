...
public class EZ_BossPortalSpawner : MonoBehaviour
{
    ...
    [Header("씬 시작 대화 잠금")]
    [Tooltip("true면 소환 타이머 진행/소환 시도 자체가 완전히 정지됩니다. 대화가 끝나면 SetSpawnLocked(false)로 해제하세요.")]
    ...
    private void SpawnMonster(GameObject prefab, Transform portal)
    {
        ...
        if (spawnSFX != null && SoundManager.Instance != null)
        {
    ...
    /// </summary>
    /// <summary>
    /// 외부(예: 씬 시작 컷씬 트리거)에서 대화 잠금 상태를 통지할 수 있게 해 주는 메소드
    /// </summary>
