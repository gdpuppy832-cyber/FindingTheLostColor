...
public class B_EnemyAttack : MonoBehaviour
{
    float telegraphTime = 2f;    // 경고 표시 시간
    ...
    void Start()
    {
        B_enemyMove = GetComponent<B_EnemyMove>();
        ...
        {
            contactHitbox.gameObject.layer = 0; // Default 레이어로 변경
    ...
    void Update()
    {
        ...
        if (B_enemyMove != null && !B_enemyMove.IsTrueGrounded)
            return;
